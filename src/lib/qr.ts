...
export function renderQrSvg(
  ...
  const mark = watermark
    ? `<g><rect x="${size / 2 - size * 0.22}" y="${size - size * 0.12}" width="${size * 0.44}" height="${size * 0.075}" rx="${size * 0.0375}" fill="${template.fg}" opacity="0.9"/><text x="${size / 2}" y="${size - size * 0.065}" text-anchor="middle" font-family="Inter, Arial, sans-serif" font-size="${size * 0.042}" fill="${template.bg}">the-qrcode-generator.com</text></g>`
    : "";
