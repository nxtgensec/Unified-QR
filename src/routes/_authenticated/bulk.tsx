...
import { renderQrSvg, svgToDataUrl, templates } from "@/lib/qr";
...
function BulkPage() {
  ...
  function renderSvg(destination: string): string {
    return renderQrSvg(destination, { templateId }, { size: 256, margin: 2 });
  }
  ...
  async function runImport() {
    ...
    if (allowed.length === 0) {
      setBusy(false);
      toast.error(`You've reached your dynamic code limit (${limit}). Upgrade your plan for more.`);
      setStep("review");
      return;
    }
  ...
  async function downloadZip() {
    ...
    for (const r of okResults) {
      const svg = renderQrSvg(shortUrl(r.slug!), { templateId }, { size: pngSize, margin: 4 });
      const img = new Image();
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-10">
      ...
      {step === "design" && (
        <div className="mt-8">
          <h2 className="text-sm font-bold uppercase tracking-wide text-muted-foreground">
            Choose a template for all codes
          </h2>
      ...
      {step === "preview" && (
        <div className="mt-8">
          <h2 className="text-sm font-bold uppercase tracking-wide text-muted-foreground">
            Preview ({rows.length} codes with template #{templateId})
          </h2>
          <div className="mt-4 grid max-h-[500px] grid-cols-2 gap-3 overflow-auto rounded-2xl border border-border bg-background p-4 shadow-card sm:grid-cols-3 lg:grid-cols-4">
            {rows.slice(0, 50).map((r, i) => (
              ...
              >
                <img
                  src={svgToDataUrl(renderSvg(r.destination))}
                  alt={r.name}
