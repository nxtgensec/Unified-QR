...
function BulkPage() {
  ...
  async function importAll() {
    ...
    if (validDestinations.length === 0) {
      setBusy(false);
      toast.error("No valid http/https URLs found in the import file.");
      return;
    }
    if (validDestinations.length < allowed.length) {
      toast.warning(`${allowed.length - validDestinations.length} row(s) skipped (invalid URLs).`);
    }

    const payload = validDestinations.map((r) => ({
      user_id: user.id,
      name: r.name,
      type: "url",
    ...
    setRows((prev) => prev.slice(allowed.length));
    if (allowed.length >= rows.length) setFileName("");
